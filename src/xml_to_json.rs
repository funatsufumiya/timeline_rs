use quickxml_to_serde::{xml_str_to_json, xml_string_to_json, Config};
...
pub fn xml_to_json(xml: &str) -> Result<serde_json::Value, minidom::error::Error> {
    let config = &*XML_CONFIG_DEFAULT;
    let json = xml_str_to_json(xml, config);
    json
}
